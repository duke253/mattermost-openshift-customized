...
if [ ! -f $MM_CONFIG ]; then
	...
	jq '.PluginSettings.Plugins.mattermost-teamcity-plugin.teamcitymaxbuilds = "5"' $MM_CONFIG > $MM_CONFIG.tmp && mv $MM_CONFIG.tmp $MM_CONFIG
	...
	jq '.PluginSettings.Plugins.PluginStates.mattermost-teamcity-plugin.Enable = "true"' $MM_CONFIG > $MM_CONFIG.tmp && mv $MM_CONFIG.tmp $MM_CONFIG
